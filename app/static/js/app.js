...
search term here" />
  <button class="btn btn-primary mb-2" @click="searchNews">Search</button> </div>
</div>
 ...
 <ul v-for="article in articles" class="news__item col col-lg-3 col-lg-6">
...
<p class= "card-subtitle"> {{ article.description }} </p>
 ...
 searchNews: function() {
 let self = this;
 fetch('https://newsapi.org/v2/everything?q='+ self.searchTerm + '&language=en&apiKey=<api>').then(function(response) {
 return response.json();
 })
 .then(function(data) {
 console.log(data);
